...
HOURS_PER_YEAR = 1460  # timesteps per file in CMIP6
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if len(ds[code].shape) == 3:  # surface level variables
                ...
                if len(np_vars[code]) < HOURS_PER_YEAR:
                    missing_data = HOURS_PER_YEAR - len(np_vars[code])
                    np_vars[code] = np.concatenate((np_vars[code], np_vars[code][-missing_data:]), axis=0)
                ...
                for level in all_levels:
                    ...
                    if len(np_vars[f"{code}_{level}"]) < HOURS_PER_YEAR:
                        missing_data = HOURS_PER_YEAR - len(np_vars[f"{code}_{level}"])
                        np_vars[f"{code}_{level}"] = np.concatenate((np_vars[f"{code}_{level}"], np_vars[f"{code}_{level}"][-missing_data:]), axis=0)
...
@click.option("--num_shards", type=int, default=2)
def main(
    path,
    variables,
    all_levels,
