...
HOURS_PER_YEAR = 14600  # timesteps per file in CMIP6
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if len(ds[code].shape) == 3:  # surface level variables
                ...
                if len(np_vars[code]) == 14597:
                    np_vars[code] = np.concatenate((np_vars[code], np_vars[code][-3:]), axis=0)
                ...
                for level in all_levels:
                    ...
                    if len(np_vars[f"{code}_{level}"]) == 14597:
                        np_vars[f"{code}_{level}"] = np.concatenate((np_vars[f"{code}_{level}"], np_vars[f"{code}_{level}"][-3:]), axis=0)
        ...
        if year == '201001010600-201501010000': # only 7300 points
            num_shards = num_shards_per_year // 2
        else:
...
@click.option("--num_shards", type=int, default=20)
def main(
    path,
    variables,
    all_levels,
