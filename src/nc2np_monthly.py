...
def main(path, variables, start_train_year, start_val_year, start_test_year, end_year):
    ...
    if len(variables) <= 3:  # small dataset for testing new models
        monthly_datapath = os.path.join(os.path.dirname(path), f"{os.path.basename(path)}_monthly_small_np")
    else:
        monthly_datapath = os.path.join(os.path.dirname(path), f"{os.path.basename(path)}_monthly_np")
