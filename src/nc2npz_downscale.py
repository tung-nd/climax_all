...
def nc2np(path_mpi, path_era5, variables, years, save_dir, partition, num_shards_per_year):
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            ds_mpi = xr.open_mfdataset(ps_mpi, combine="by_coords", parallel=True)  # dataset for a single variable
            ...
            if len(ds_mpi[code].shape) == 3:  # surface level variables
                ...
                np_mpi_vars[code] = ds_mpi[code].to_numpy()[:HOURS_PER_YEAR]
                np_era5_vars[code] = ds_era5[code].to_numpy()[:HOURS_PER_YEAR]

                if partition == "train":  # compute mean and std of each var in each year
                    var_mean_mpi_yearly = np_mpi_vars[code].mean(axis=(0, 2, 3))
                    var_std_mpi_yearly = np_mpi_vars[code].std(axis=(0, 2, 3))
                    var_mean_era5_yearly = np_era5_vars[code].mean(axis=(0, 2, 3))
                    var_std_era5_yearly = np_era5_vars[code].std(axis=(0, 2, 3))
                ...
                for level in all_levels:
                    ...
                    level = int(level)
                    # remove the last 24 hours if this year has 366 days
                    np_mpi_vars[f"{code}_{level}"] = ds_level_mpi[code].to_numpy()[:HOURS_PER_YEAR]
                    np_era5_vars[f"{code}_{level}"] = ds_level_era5[code].to_numpy()[:HOURS_PER_YEAR]

                    if partition == "train":  # compute mean and std of each var in each year
                        var_mean_mpi_yearly = np_mpi_vars[f"{code}_{level}"].mean(axis=(0, 2, 3))
                        var_std_mpi_yearly = np_mpi_vars[f"{code}_{level}"].std(axis=(0, 2, 3))
                        var_mean_era5_yearly = np_era5_vars[f"{code}_{level}"].mean(axis=(0, 2, 3))
                        var_std_era5_yearly = np_era5_vars[f"{code}_{level}"].std(axis=(0, 2, 3))
                        if var not in normalize_mean_input:
...
@click.command()
@click.option("--path_mpi", type=str, default='/datadrive/datasets/CMIP6/MPI-ESM/5.625deg_aligned')
@click.option("--path_era5", type=str, default='/datadrive/datasets/1.40625deg_aligned')
@click.option("--save_dir", type=str, default='/datadrive/datasets/mpi_era5_downscale_5.625_1.40625')
@click.option(
    "--variables",
    "-v",
    type=click.STRING,
