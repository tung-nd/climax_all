...
def get_data_traj(dataset, start_idx, steps):
    ...
    return get_seq_imgs(dataset_traj, "Data")
...
def get_model_traj(model, dataset, start_idx, steps):
    ...
    return get_seq_imgs(pred_traj, "Model")
...
def get_seq_imgs(data_traj, caption):
    ...
    all_steps = []
    for i in range(steps):
        fig, axes = plt.subplots(3,1, figsize=(10, 12)) # Caution, figsize will also influence positions.
...
class MyLightningCLI(LightningCLI):
    def add_arguments_to_parser(self, parser):
        ...
        parser.add_argument("--save_dir", type=str, default="/home/t-tungnguyen/climate_pretraining/visualization_vit")
...
def main(model, dataset, args):
    ...
    start_idx = np.random.randint(low=0, high=len(dataset) - steps) # choose a random index from the dataset

    data_traj = get_data_traj(dataset, start_idx, steps) # ground truth
    pred_traj = get_model_traj(model, dataset, start_idx, steps) # prediction
    traj = [np.concatenate((data_traj[i], pred_traj[i]), axis=1) for i in range(len(data_traj))]
    os.makedirs(args.save_dir, exist_ok=True)
