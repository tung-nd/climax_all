...
def nc2zarr(path, variables, years, save_dir, partition):
    ...
    for year in tqdm(years):
        ...
        yearly_dataset.to_zarr(
            os.path.join(save_dir, partition, f"{year}.zarr"),
            mode="w",
        )
...
def main(path, variables, start_train_year, start_val_year, start_test_year, end_year):
    ...
    if len(variables) <= 3:  # small dataset for testing new models
        yearly_datapath = os.path.join(os.path.dirname(path), f"{os.path.basename(path)}_yearly_small")
    else:
        yearly_datapath = os.path.join(os.path.dirname(path), f"{os.path.basename(path)}_yearly")
